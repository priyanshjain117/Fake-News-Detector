...
try:
    ...
    # It's usually better to raise the error or ensure the files are present.
    model = None
    vectorizer = None
...
def predict():
    try:
        ...
        # Clean and transform text
        cleaned_text = clean_text(text)
        # Note: The vectorizer.transform() call is thread-safe and correct here.
        vect_text = vectorizer.transform([cleaned_text])
        ...
        # Ensure error messages are useful for debugging
        print(f"Error in prediction: {str(e)}")
        # Note: Avoid exposing detailed internal errors to the user
        return jsonify({'error': 'Internal server error during prediction.'}), 500

# -------------------------------------------------
# 🔹 Run app
# -------------------------------------------------
# CHANGE: Only run the app.run() command for local development/debugging.
# When running with Gunicorn, this block is skipped, and Gunicorn imports the 'app' object directly.
if __name__ == '__main__':
    app.run(debug=True)
